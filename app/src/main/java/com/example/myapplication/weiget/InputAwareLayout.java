...
public class InputAwareLayout extends KeyboardAwareLinearLayout implements KeyboardAwareLinearLayout.OnKeyboardShownListener {
    ...
    public void show(@NonNull final EditText imeTarget, @NonNull final InputView input,final int wantHeight) {
        if (isKeyboardOpen()) {
            hideSoftkey(imeTarget, new Runnable() {
                @Override
                public void run() {
                    hideAttachedInput(true);
                    input.show(wantHeight, true);
                    current = input;
                }
            });
        } else {
            if (current != null) current.hide(true);
            input.show(wantHeight, current != null);
            current = input;
        }
    }
    ...
    public void showSoftkey(final EditText inputTarget) {
        ...
        inputTarget.post(new Runnable() {
            ...
            public void run() {
                ...
                keyBordUtils.getInputMethodManager(inputTarget.getContext()).showSoftInput(inputTarget, 0);
            }
        });
    }
    ...
    public void hideSoftkey(final EditText inputTarget, @Nullable Runnable runAfterClose) {
        ...
        keyBordUtils.getInputMethodManager(inputTarget.getContext())
